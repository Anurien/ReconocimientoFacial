from cv2 import cv2
import numpy as np
#se meten los puntos para detectar la forma
def ordenarpuntos(puntos):
    n_puntos=np.concatenate([puntos[0],puntos[1],puntos[2],puntos[3]]).tolist()
    #ordenamos los puntos porque el concatenate no lo hace 
...
#pasar a escala de grises quitar ruido y umbral
def alineamiento(imagen,ancho,alto):
    ...
    for c in contorno:
        ...
        if len(approximacion)==4:
            ...
            #estos son los puntos del papel (hace falta un fondo blanco)
            puntos2=np.float32([[0,0],[ancho,0],[0,alto],[ancho,alto]])
            #si mueves el movil que no se pierdan los datos
            M = cv2.getPerspectiveTransform(puntos1, puntos2)
            imagen_alineada=cv2.warpPerspective(imagen, M, (ancho,alto))
    return imagen_alineada

# para capturar video (o en movil 1 en camara externa)
...
while True:
    ...
    if imagen_A6 is not None:
        puntos=[]
        #aqui hacemos lo de los anteriores ejercicios pasar a escala de grises quitar ruido y umbral
